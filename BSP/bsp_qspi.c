...
//	QSPI_W25Qxx_Reset();							// 复位器件
	...
	if( Device_ID == W25Qxx_FLASH_ID )		// 进行匹配	0Xef4017 
	{
...
int8_t QSPI_W25Qxx_WriteEnable(void)
{
	...
	if (HAL_QSPI_Command(&hqspi, &s_command, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) 
	{
		printf("W25Qxx_ERROR_WriteEnable\r\n");
...
// 读状态寄存器1的第1位（只读），WEL写使能标志位，该标志位为1时，代表可以进行写操作
	...
	if (HAL_QSPI_AutoPolling(&hqspi, &s_command, &s_config, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
	{
		printf("W25Qxx_ERROR_AUTOPOLLING\r\n");
		return W25Qxx_ERROR_AUTOPOLLING; 	// 轮询等待无响应
	}	
	return QSPI_W25Qxx_OK;  // 通信正常结束
}
...
void qspi_flash_test(void){
	...
	QSPI_W25Qxx_SectorErase(0x00000000*0x1000);
	
	QSPI_W25Qxx_WriteBuffer(buff_data,0x00000000,sizeof(buff_data));
	
	QSPI_W25Qxx_ReadBuffer(read_data,0x00000000,sizeof(buff_data));
	
    if( 0 == memcmp(buff_data,read_data,sizeof(buff_data))){
		printf("\r\n qspi flash data check success \r\n");
	
	}else{
	
	    printf("\r\n qspi flash data check failed \r\n");
	}

}


#define FILENAME "0:test.txt"
#define BUFFER_SIZE 128



     FATFS fs; // 定义一个 FATFS 类型的变量
    FIL file; // 文件对象
    FRESULT result; // 操作结果
    char buffer_write[BUFFER_SIZE] = "Hello, FatFs!"; // 要写入的数据
    char buffer_read[BUFFER_SIZE]; // 用于存储读取的数据
	MKFS_PARM opt;
	BYTE work[512]; 
...
// 执行格式化操作
//    result = f_mkfs("0:", &opt, NULL, 4096);
//	
//	
//	
//    if (result != FR_OK) {
//        printf("Failed to format filesystem. Error code: %d\n", result);
//        return 1;
//    }else {
...
//	}
    ...
    // 写入数据到文件
    UINT bytes_written;
    result = f_write(&file, buffer_write, strlen(buffer_write), &bytes_written);
    if (result != FR_OK) {
    ...
    // 读取文件内容
    UINT bytes_read;
    result = f_read(&file, buffer_read, sizeof(buffer_read), &bytes_read);
    if (result != FR_OK) {
